import { Activity, Bot, RefreshCcw } from 'lucide-react';
...
export function Agents() {
  const { agents, runHealthCheck } = useAdminData();
  return (
    <div className="page-stack">
      <PageHeader
        title="Agent Health"
        description="Monitor each category agent, APIM/webhook URL health, request volume, errors and latency."
        actions={<button className="primary-btn" onClick={runHealthCheck}><Activity size={16} /> Run Health Check</button>}
      />
      ...
      <section className="panel">
        ...
        <DataTable
          columns={['Agent', 'Category', 'Status', 'Uptime', 'Requests today', 'Errors today', 'Avg latency', 'Last checked', 'Action']}
          rows={agents.map((agent) => [
            ...
            <button className="secondary-btn" onClick={runHealthCheck}><RefreshCcw size={15} /> Recheck</button>,
          ])}
        />
      </section>
    </div>
